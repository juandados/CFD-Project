function [U,V,f1,f2]=projection_force(nx, ny)
...
ly = 1;       % height of box
if nargin < 1
nx = 80;      % number of x-gridpoints
end
if nargin < 1
ny = 80;      % number of y-gridpoints
end
...
for k = 1:nt
   ...
   t = k*dt;
   uN = x*0;      vN = (avg(x).^2-avg(x))*t;
   uS = x*0;      vS = (avg(x).^2-avg(x))*t;
   uW = (avg(y).^2-avg(y))*t; vW = y*0;
   uE = (avg(y).^2-avg(y))*t; vE = y*0;
