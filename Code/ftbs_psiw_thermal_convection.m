function [xx,yy,Us,Vs,Ts,Cs,Ws,dt,tend,tS,Re,Pr,Ra,Pe] = ftbs_psiw_thermal_convection(nsteps,prob,nx,ny)
...
for i = 1 : nt,
  ...
  % scheme. 
  w(jj,ii) = w(jj,ii) - dt * ...
      ((u(jj,ii)./(abs(u(jj,ii))+eps)).*u(jj,ii) .* (w(jj,ii)-w(jj,ii-1)) / (dx) ...
	+ (v(jj,ii)./(abs(v(jj,ii))+eps)).*v(jj,ii) .* (w(jj,ii)-w(jj-1,ii)) / (dy) ) ...
	+ (1/Re)*dt*(w(jj,ii+1) - 2*w(jj,ii) + w(jj,ii-1)) / dx^2 ...
	+ (1/Re)*dt*(w(jj+1,ii) - 2*w(jj,ii) + w(jj-1,ii)) / dy^2 ...
    + dt*b*g*(T(jj,ii)-T(jj,ii-1)) / (dx);
